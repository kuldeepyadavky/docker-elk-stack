...
function wait_for_elasticsearch {
	...
	if [[ -n "${ELASTIC_PASSWORD:-}" ]]; then
		...
		log "Using password authentication for user 'elastic'"
	else
		err "ELASTIC_PASSWORD environment variable is not set!"
	fi
	...
	for _ in $(seq 1 60); do
		...
		if ((exit_code)); then
			result=$exit_code
			err "Curl request failed with exit code $exit_code"
		fi
		...
		if [[ "${output: -3}" -eq 200 ]]; then
			...
			err "Received HTTP response code ${output: -3} from Elasticsearch"
		fi
	...
	if ((result)) && [[ "${output: -3}" -ne 000 ]]; then
		err "Elasticsearch returned an error: ${output::-3}"
	fi
